...
impl Discoverable for Checkbox {
    fn schema(&self) -> WidgetSchema {
        let mut schema = WidgetSchema::new("Checkbox", "A toggleable checkbox", SemanticRole::Input);
    ...
    fn capabilities(&self) -> Vec<AgentCapability> {
        vec![AgentCapability::Toggleable { state: self.checked }, AgentCapability::Focusable]
    }
...
impl Widget for Checkbox {
    fn render(self, area: Rect, frame: &mut Frame<'_>) {
        ...
        frame.painter().stroke_rect(box_rect, Color::WHITE, 1.0, 2.0);
        if self.checked {
