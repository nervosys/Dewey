...
mod tests {
    ...
    #[test]
    fn widget_schema_new() {
        let ws = WidgetSchema::new("Button", "A clickable button", SemanticRole::Action);
        assert_eq!(ws.name, "Button");
        assert_eq!(ws.default_role, SemanticRole::Action);
        assert!(ws.properties.is_empty());
        assert!(ws.actions.is_empty());
        assert!(ws.tags.is_empty());
    }

    #[test]
    fn property_schema_new() {
        let ps = PropertySchema::new("text", "The text content", PropertyType::String, true);
        assert_eq!(ps.name, "text");
        assert!(ps.required);
        assert!(ps.default_value.is_none());
        assert!(ps.constraints.is_empty());
    }

    #[test]
    fn semantic_role_display() {
        assert_eq!(format!("{}", SemanticRole::Display), "display");
        assert_eq!(format!("{}", SemanticRole::Input), "input");
        assert_eq!(format!("{}", SemanticRole::Action), "action");
        assert_eq!(format!("{}", SemanticRole::Container), "container");
        assert_eq!(
            format!("{}", SemanticRole::DataVisualization),
            "data-visualization"
        );
        assert_eq!(format!("{}", SemanticRole::Custom), "custom");
    }

    #[test]
    fn widget_schema_serialize_roundtrip() {
        let mut ws = WidgetSchema::new("Slider", "A range slider", SemanticRole::Input);
        ws.properties.push(PropertySchema::new(
            "value",
            "Current value",
            PropertyType::Float,
            true,
        ));
        ws.tags = vec!["slider".into(), "range".into()];

        let json = serde_json::to_string(&ws).unwrap();
        let ws2: WidgetSchema = serde_json::from_str(&json).unwrap();
        assert_eq!(ws2.name, "Slider");
        assert_eq!(ws2.properties.len(), 1);
        assert_eq!(ws2.tags.len(), 2);
    }

    #[test]
    fn property_type_nested() {
        let arr = PropertyType::Array(Box::new(PropertyType::String));
        let json = serde_json::to_string(&arr).unwrap();
        let arr2: PropertyType = serde_json::from_str(&json).unwrap();
        assert_eq!(arr, arr2);
    }

    #[test]
    fn property_constraint_serialize() {
        let constraints = vec![
            PropertyConstraint::Min(0.0),
            PropertyConstraint::Max(100.0),
            PropertyConstraint::MinLength(1),
            PropertyConstraint::MaxLength(255),
            PropertyConstraint::Pattern(r"^\d+$".into()),
        ];
        for c in constraints {
            let json = serde_json::to_string(&c).unwrap();
            let c2: PropertyConstraint = serde_json::from_str(&json).unwrap();
            assert_eq!(c, c2);
        }
    }
}
